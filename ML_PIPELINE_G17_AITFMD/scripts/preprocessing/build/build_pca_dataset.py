"""Build PCA-transformed dataset from avg3 cubes using fitted PCA model."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build PCA-transformed cubes from avg3 dataset.")
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            f"PCA model not found: {model_path}. Run fit_pca.py first."
        )
    ...
    if mask_root:
        print(
            f"[pca build] mask: root={mask_root} require={require_mask} apply_to_cube={apply_mask}"
        )
    ...
    if not verbose:
        iterator = tqdm(iterator, total=len(split_df), desc="build_pca", unit="roi")
    ...
    print(f"[pca] cubes written: {written}")
